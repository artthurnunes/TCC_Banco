...
-- ------------------------------------------------------- SELECTS COM PROBLEMA NAS TELAS
  ...
  --  select para a tela de pagamento de mensalidade, esta retornando registros repeditos com valores de mensalidade diferentes
 -- se funcionar, usar nos botoes frente e traz também da tela
...
WHERE NOME LIKE '%%' AND C.CD_PLANO_ALUNO = (SELECT MAX(CD_PLANO_ALUNO) FROM TB_PLANOS_ALUNOS)
GROUP BY A.CD_REGISTRO,A.NOME,C.VALOR;
-- ORDER BY A.NOME ASC, MAX(C.CD_PLANO_ALUNO) DESC;


-- ESTA TRAZENDO VALOR DE PLANO REPETIDO QUANDO TEM MAIS DE UM REGISTRO
SELECT 				A.CD_REGISTRO,A.NOME,A.TEL1, DATE_FORMAT(D.DT_ENTRADA,'%e/%m/%Y')
