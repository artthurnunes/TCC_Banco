...
SELECT A.CD_REGISTRO,A.NOME,A.TEL1, DATE_FORMAT(D.DT_ENTRADA,'%e/%m/%Y'),(SELECT VALOR FROM tb_planos_alunos ORDER BY CD_PLANO_ALUNO DESC LIMIT 1),MAX(DATE_FORMAT(C.PROXIMO_VENCIMENTO,'%e/%m/%Y')) FROM TB_ALUNOS A INNER JOIN TB_PLANOS_ALUNOS B ON B.CD_REGISTRO = A.CD_REGISTRO INNER JOIN TB_HISTORICO_PAGAMENTOS_ALUNOS C ON C.CD_REGISTRO = A.CD_REGISTRO LEFT JOIN TB_FREQUENCIA_ALUNOS D ON D.CD_REGISTRO = A.CD_REGISTRO WHERE C.PROXIMO_VENCIMENTO < NOW() GROUP BY A.CD_REGISTRO,A.NOME,A.TEL1,D.DT_ENTRADA;
