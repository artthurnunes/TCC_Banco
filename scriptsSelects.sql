...
SELECT * FROM TB_FOTOS_ALUNOS;

SELECT ESQ_SENHA FROM TB_SENHAS WHERE USUARIO = '';

SELECT B.NOME, MAX(C.DT_FIM) 
...
WHERE B.SITUACAO = 1 AND DATEDIFF(NOW(),A.DT_ENTRADA) < 8 
GROUP BY A.CD_REGISTRO;

SELECT A.NOME, STR_TO_DATE(MAX(B.DT_FIM),'%e/%m/%Y') 
FROM TB_ALUNOS A 
INNER JOIN TB_TREINOSA B ON B.CD_REGISTRO = A.CD_REGISTRO -- AND STR_TO_DATE(B.DT_FIM,'%e/%m/%Y') < NOW() 
WHERE A.SITUACAO = 1 
GROUP BY A.NOME;

SELECT MAX(DT_FIM) FROM TB_TREINOSA WHERE DT_FIM < NOW();

INSERT INTO TB_FOTOS_ALUNOS (CD_REGISTRO)VALUE(3);

UPDATE TB_FOTOS_ALUNOS SET IMAGEM = '' WHERE CD_REGISTRO = '' 

UPDATE TB_HISTORICO_PAGAMENTOS_ALUNOS SET PROXIMO_VENCIMENTO = '2018-01-01' WHERE CD_REGISTRO = 2;

SELECT A.CD_REGISTRO,A.NOME,A.TEL1, DATE_FORMAT(D.DT_ENTRADA,'%e/%m/%Y'),B.VALOR,MAX(DATE_FORMAT(C.PROXIMO_VENCIMENTO,'%e/%m/%Y')) FROM TB_ALUNOS A INNER JOIN TB_PLANOS_ALUNOS B ON B.CD_REGISTRO = A.CD_REGISTRO AND SN_ATIVO = 1 INNER JOIN TB_HISTORICO_PAGAMENTOS_ALUNOS C ON C.CD_REGISTRO = A.CD_REGISTRO LEFT JOIN TB_FREQUENCIA_ALUNOS D ON D.CD_REGISTRO = A.CD_REGISTRO WHERE C.PROXIMO_VENCIMENTO < NOW() AND C.DT_PAGAMENTO IS NULL GROUP BY A.CD_REGISTRO,A.NOME,A.TEL1,D.DT_ENTRADA,B.VALOR; 
        


SELECT * FROM TB_PLANOS_ALUNOS WHERE CD_REGISTRO = 1 AND SN_ATIVO = 1;

-- SELECT PARA TELA DE MENSALIDADE. PUXA TODOS OS ALUNOS, USAR NO BOTÃO FRENTE E TRAZ
...
SELECT A.CD_REGISTRO,A.NOME,MAX(DATE_FORMAT(C.PROXIMO_VENCIMENTO,'%e/%m/%Y')) FROM TB_ALUNOS A INNER JOIN TB_HISTORICO_PAGAMENTOS_ALUNOS C ON C.CD_REGISTRO = A.CD_REGISTRO WHERE C.PROXIMO_VENCIMENTO < NOW() AND C.DT_PAGAMENTO IS NULL GROUP BY A.CD_REGISTRO,A.NOME;

SELECT SUM(VALOR) FROM TB_DESPESAS_PROGRAMADAS WHERE DATA_PAGAMENTO BETWEEN '2018-10-1' AND '2018-10-31' AND PAGO = 1;

SELECT SUM(VALOR_PAGO) FROM TB_HISTORICO_PAGAMENTOS_ALUNOS WHERE DT_PAGAMENTO BETWEEN '2018-10-1' AND '2018-10-31';
...
UPDATE TB_HISTORICO_PAGAMENTOS_ALUNOS SET PROXIMO_VENCIMENTO = '2018-09-10' WHERE CD_PAGAMENTO = 4;
...
CREATE OR REPLACE VIEW V_MAX_CD_PLANO AS 
	SELECT CD_REGISTRO
           ,MAX(CD_TREINOA) AS MAX2
		FROM TB_TREINOSA WHERE (STR_TO_DATE((DT_FIM),'%e/%m/%Y')<NOW())
	GROUP BY CD_REGISTRO;
    
SELECT A.NOME, STR_TO_DATE(MAX(B.DT_FIM),'%e/%m/%Y') 
FROM TB_ALUNOS A 
INNER JOIN TB_TREINOSA B ON B.CD_REGISTRO = A.CD_REGISTRO
INNER JOIN V_MAX_CD_PLANO V ON B.CD_REGISTRO = V.cd_registro and B.CD_TREINOA = v.max2
WHERE A.SITUACAO = 1 
GROUP BY A.NOME;    
    
SELECT 			 A.CD_REGISTRO
				,A.NOME
                ,MAX(B.PROXIMO_VENCIMENTO) 
                ,C.VALOR
	FROM TB_ALUNOS A
		INNER JOIN TB_HISTORICO_PAGAMENTOS_ALUNOS B ON B.CD_REGISTRO = A.CD_REGISTRO
		INNER JOIN TB_PLANOS_ALUNOS C ON C.CD_REGISTRO = A.CD_REGISTRO 
